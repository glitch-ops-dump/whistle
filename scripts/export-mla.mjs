...
const mlaHtmlPath = path.join(dist, "mla.html");
const outputPath = path.join(root, "exports", "standalone", "whistle-mla-mockup.html");
...
if (!fs.existsSync(mlaHtmlPath)) {
  throw new Error("dist/mla.html not found. Run npm run build before exporting.");
}

let html = read(mlaHtmlPath);
...
html = html.replace(/<script type="module" src="\/src\/mla-main\.tsx"><\/script>/, "");
