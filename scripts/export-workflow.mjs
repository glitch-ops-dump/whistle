...
const htmlPath = path.join(dist, "workflow.html");
const outputPath = path.join(root, "exports", "standalone", "whistle-workflow-infographic.html");
...
if (!fs.existsSync(htmlPath)) {
  throw new Error("dist/workflow.html not found. Run npm run build before exporting.");
}
...
window.__WHISTLE_WORKFLOW_ASSETS__ = {
  ...
  civic: "${dataUri(path.join(root, "public", "assets", "brand", "whistle-civic-mark.svg"), "image/svg+xml")}"
};
</script>`;

html = html.replace("<body>", `<body>\n    ${runtimeScript}`);
html = html.replace(/<script type="module" src="\/src\/workflow-main\.tsx"><\/script>/, "");
