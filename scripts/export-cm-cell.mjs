...
const cmCellHtmlPath = path.join(dist, "cm-cell.html");
const outputPath = path.join(root, "exports", "standalone", "whistle-cm-cell-mockup.html");
...
if (!fs.existsSync(cmCellHtmlPath)) {
  throw new Error("dist/cm-cell.html not found. Run npm run build before exporting.");
}

let html = read(cmCellHtmlPath);
...
html = html.replace(/<script type="module" src="\/src\/cm-cell-main\.tsx"><\/script>/, "");
